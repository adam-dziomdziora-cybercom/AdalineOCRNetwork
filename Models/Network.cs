...
{
    private readonly int letterSize;

    public IList<Neuron> Neurons { get; private set; } = new List<Neuron>();

    public Network(string lettersDirectory)
    {
        var path = Path.Combine(Environment.CurrentDirectory, lettersDirectory);
        var letterFilePaths = Directory.EnumerateFiles(path, "*.txt");
        ...
        {
            letterSize = Neurons.FirstOrDefault()?.Weights.Length ?? 0;
        }
        else
        {
